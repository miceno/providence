	...
	trait CLIUtilsImportExport { 
		...
		public static function import_mediaParamList() {
			...
			return array(
				...
				'allow-duplicate-media|D-s' => _t('Import media even if it already exists in CollectiveAccess. Default is false – skip import of duplicate media.'),
				...
				'import-target-type-s' => _t('Type to use for all newly created target records. Default is the first type in the target\'s type list.'),
				'import-target-idno-s' => _t('Identifier to use for all newly created target records.'),
				...
				'import-target-access-s' => _t('Set access for newly created target records. Possible values are %1. Default is %2.', $access_status_list_str, $access_status_default_str),
				'import-target-status-s' => _t('Set status for newly created target records. Possible values are %1. Default is %2.', $workflow_status_list_str, $workflow_status_default_str),
				'representation-type-s' => _t('Type to use for all newly created representations. Possible values are %1. Default is %2.', $representation_type_list_str, $representation_type_default_str),
				'representation-idno-s' => _t('Identifier to use for all newly created representation records.'),
				'representation-idno-mode-s' => _t('Sets how identifiers of newly created representations are set. Valid values are AUTO, FILENAME, FILENAME_NO_EXT, DIRECTORY_AND_FILENAME. Set to AUTO to use an identifier calculated according to system numbering settings; set to FILENAME to use the file name as identifier; set to FILENAME_NO_EXT to use the file name stripped of extension as the identifier; use DIRECTORY_AND_FILENAME to set the identifer to the directory name and file name with extension. Default is AUTO.'),
				'representation-access-s' => _t('Set access for newly created representations. Possible values are %1. Default is %2.', $access_status_list_str, $access_status_default_str),
				'representation-status-s' => _t('Set status for newly created representations. Possible values are %1. Default is %2.', $workflow_status_list_str, $workflow_status_default_str),
				'representation-mapping-s' => _t('Code for mapping to apply when importing media.'),
				'remove-media-on-import|r-s' => _t('Remove media from directory after it has been successfully imported. Default is false.')
			);
		}
		# -------------------------------------------------------
		/**
