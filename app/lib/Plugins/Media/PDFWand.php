	...
	*/ 
	...
	public function read ($ps_filepath, $mimetype="", $options=null) {
		...
		if (caPDFMinerInstalled($this->ops_pdfminer_path)) {
			...
			caExecExpected($this->ops_pdfminer_path.' -t text '.caEscapeShellArg($ps_filepath).' > '.caEscapeShellArg($vs_tmp_filename).(caIsPOSIX() ? " 2> /dev/null" : ""));
			...
			caExecExpected($this->ops_pdfminer_path.' -A -t xml '.caEscapeShellArg($ps_filepath).' > '.caEscapeShellArg($vs_tmp_filename).(caIsPOSIX() ? " 2> /dev/null" : ""));
			...
			if (caMediaPluginPdftotextInstalled($this->ops_pdftotext_path)) {
				$vs_tmp_filename = tempnam('/tmp', 'CA_PDF_TEXT');
				caExecExpected($this->ops_pdftotext_path.' -q -enc UTF-8 '.caEscapeShellArg($ps_filepath).' '.caEscapeShellArg($vs_tmp_filename).(caIsPOSIX() ? " 2> /dev/null" : ""));
				$vs_extracted_text = file_get_contents($vs_tmp_filename);
				$this->handle['content'] = $this->ohandle['content'] = $vs_extracted_text;
				@unlink($vs_tmp_filename);
			}
