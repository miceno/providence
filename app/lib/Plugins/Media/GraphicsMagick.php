...
class WLPlugMediaGraphicsMagick Extends BaseMediaPlugin Implements IWLPlugMedia {
	...
	public function &writePreviews($ps_filepath, $pa_options) {
		...
		$va_output = array();
		if(!caExecExpected($this->ops_graphicsmagick_path.' identify -format "%m\n" '.caEscapeShellArg($this->filepath).(caIsPOSIX() ? " 2> /dev/null" : ""), $va_output)){
			return false;
		}
		...
		if(!caExecExpected($this->ops_graphicsmagick_path.' convert '.caEscapeShellArg($this->filepath)." +adjoin ".$vs_output_file.(caIsPOSIX() ? " 2> /dev/null" : ""), $va_output)){
			return false;
		}
	...
	public function joinArchiveContents($pa_files, $pa_options = array()) {
		...
		if(sizeof($va_acceptable_files)){
			if(caExecExpected($this->ops_graphicsmagick_path." convert ".join(" ",$va_acceptable_files)." ".$vs_archive_original.(caIsPOSIX() ? " 2> /dev/null" : ""), $va_output, 1)){
	...
	private function _graphicsMagickIdentify($ps_filepath) {
		if (caMediaPluginGraphicsMagickInstalled($this->ops_graphicsmagick_path)) {
			if(caExecExpected($this->ops_graphicsmagick_path.' identify -format "%m;" '.caEscapeShellArg($ps_filepath.'[0]').(caIsPOSIX() ? " 2> /dev/null" : ""), $va_output)){
	...
	private function _graphicsMagickGetMetadata($ps_filepath, $options=null) {
		...
		if(!isset($va_metadata['EXIF']) || !is_array($va_metadata['EXIF'])) {
			if ( ! caExecExpected( $this->ops_graphicsmagick_path . ' identify -format "%[EXIF:*]" '
		...
		$vs_iptc_file .= '.iptc';
		if(!caExecExpected($this->ops_graphicsmagick_path." convert ".caEscapeShellArg($ps_filepath)." ".caEscapeShellArg($vs_iptc_file).(caIsPOSIX() ? " 2> /dev/null" : ""), $va_output, 1)){
			return null;
		}
		...
		/* DPX metadata */
		if(!caExecExpected($this->ops_graphicsmagick_path." identify -format '%[DPX:*]' ".caEscapeShellArg($ps_filepath).(caIsPOSIX() ? " 2> /dev/null" : ""), $va_output)){
			return null;
		}
	...
	private function _graphicsMagickRead($ps_filepath) {
		if (caMediaPluginGraphicsMagickInstalled($this->ops_graphicsmagick_path)) {
			...
			if( !($this->metadata = $this->_graphicsMagickGetMetadata($ps_filepath) )){
				return null;
			}
	...
	private function _graphicsMagickWrite($pa_handle, $ps_filepath, $ps_mimetype, $pn_quality=null) {
		if (caMediaPluginGraphicsMagickInstalled($this->ops_graphicsmagick_path)) {
			...
			if (is_array($va_ops['convert']) && sizeof($va_ops['convert'])) {
				...
				if(!caExecExpected($this->ops_graphicsmagick_path.' convert '.caEscapeShellArg($vs_input_file.'[0]').' '.join(' ', $va_ops['convert']).' '.caEscapeShellArg($ps_filepath).(caIsPOSIX() ? " 2> /dev/null" : ""), $_, 1)){
					return null;
				}
				$vs_input_file = $ps_filepath;
			}
			if (is_array($va_ops['composite']) && sizeof($va_ops['composite'])) {
				if(!caExecExpected($this->ops_graphicsmagick_path.' composite '.join(' ', $va_ops['composite']).' '.caEscapeShellArg($vs_input_file.'[0]').' '.caEscapeShellArg($ps_filepath).(caIsPOSIX() ? " 2> /dev/null" : ""), $_, 1)){
					return null;
				}
			}	
